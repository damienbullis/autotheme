...
:root {
  ...
  --primary-200: oklch(from var(--base) 0.87 calc(c * 0.40) h);
  --primary-300: oklch(from var(--base) 0.80 calc(c * 0.60) h);
  --primary-400: oklch(from var(--base) 0.70 calc(c * 0.80) h);
  ...
  --primary-800: oklch(from var(--base) calc(l - 0.20) calc(c * 0.80) h);
  --primary-900: oklch(from var(--base) calc(l - 0.27) calc(c * 0.70) h);
  ...
  --primary-tone-1: oklch(from var(--base) l calc(c * 0.20) h);
  --primary-tone-2: oklch(from var(--base) l calc(c * 0.40) h);
  --primary-tone-3: oklch(from var(--base) l calc(c * 0.60) h);
  --primary-tone-4: oklch(from var(--base) l calc(c * 0.80) h);
}
```

**Why this matters:** Change `--base` at runtime and the *entire* theme updates instantly. No JavaScript palette computation. No theme switching delay. A color picker directly connected to `--base` gives you a live theme editor for free.
...
AutoTheme wouldn't need to compute every color -- it would compute the *relationships*. The tint curve, the chroma falloff, the harmony angles. Then emit those as CSS expressions. The browser does the math.
...
This produces harmony colors that *feel* equally bold, even when their absolute chroma differs.
...
:root {
  ...
  --surface: light-dark(
    oklch(0.97 0.005 250),    /* Light mode */
    oklch(0.12 0.008 250)     /* Dark mode */
  );

  --surface-elevated: light-dark(
    oklch(1.00 0.000 0),
    oklch(0.16 0.010 250)
  );

  --text-1: light-dark(
    oklch(0.15 0.010 250),
    oklch(0.93 0.010 250)
  );
  ...
  --primary: light-dark(
    var(--base),
    oklch(from var(--base) calc(l + 0.10) c h)  /* Lightened for dark mode */
  );
}
```
...
:root {
  --theme-hue: 250;
  transition: --theme-hue 0.6s ease, --theme-lightness 0.4s ease;
}
...
@property --at-hue { syntax: "<number>"; inherits: true; initial-value: 250; }
@property --at-chroma { syntax: "<number>"; inherits: true; initial-value: 0.20; }
@property --at-lightness { syntax: "<number>"; inherits: true; initial-value: 0.55; }
@property --at-darkness { syntax: "<number>"; inherits: true; initial-value: 0.12; }
@property --at-text-levels { syntax: "<integer>"; inherits: true; initial-value: 5; }
```
...
:root {
  --surface: oklch(0.97 0.000 0);         /* True achromatic */
  --text-1: oklch(0.15 0.000 0);
}
```
...
:root {
  ...
  --ease-spring: linear(
    0, 0.006, 0.025, 0.054, 0.095, 0.146, 0.206, 0.272,
    0.343, 0.418, 0.494, 0.569, 0.642, 0.710, 0.773, 0.830,
    0.879, 0.921, 0.955, 0.981, 1.0, 1.012, 1.019, 1.021,
    1.019, 1.014, 1.008, 1.003, 1.0
  );
...
```css
:root {
  --transition-colors: color var(--duration-3) var(--ease-color),
                       background-color var(--duration-3) var(--ease-color),
...
.compact-region { --density: compact; }
.comfortable-region { --density: comfortable; }

/* Components adapt automatically */
@container style(--density: compact) {
  .card { padding: var(--spacing-2); }
  .button { padding: var(--spacing-1) var(--spacing-3); font-size: var(--text-sm); }
}

@container style(--density: comfortable) {
  .card { padding: var(--spacing-6); }
  .button { padding: var(--spacing-3) var(--spacing-6); font-size: var(--text-md); }
}
```
...
```css
[data-theme-variant="vibrant"] { --theme-variant: vibrant; }
[data-theme-variant="muted"] { --theme-variant: muted; }
...
@media (prefers-contrast: more) {
  :root {
    ...
    --border-width: 2px;
    --surface: oklch(0.99 0.000 0);
    --text-1: oklch(0.05 0.000 0);
...
@media (prefers-contrast: less) {
  :root {
    ...
    --text-1: oklch(0.30 0.008 var(--theme-hue));
  }
}
```
...
@media (prefers-reduced-transparency) {
  :root {
    ...
    --surface-overlay: oklch(0.20 0.005 var(--theme-hue)); /* Was scrim */
  }
}
```
...
```css
:root {
  --shadow-color: oklch(0.10 0.02 var(--theme-hue));

  --shadow-1: 0 1px 2px oklch(from var(--shadow-color) l c h / 8%);
  --shadow-2: 0 2px 4px oklch(from var(--shadow-color) l c h / 10%),
              0 1px 2px oklch(from var(--shadow-color) l c h / 6%);
  --shadow-3: 0 4px 8px oklch(from var(--shadow-color) l c h / 12%),
              0 2px 4px oklch(from var(--shadow-color) l c h / 8%);
  --shadow-4: 0 8px 16px oklch(from var(--shadow-color) l c h / 14%),
              0 4px 8px oklch(from var(--shadow-color) l c h / 10%);
  --shadow-5: 0 16px 32px oklch(from var(--shadow-color) l c h / 16%),
...
@media (prefers-reduced-motion: reduce) {
  :root { --duration-1: 0ms; --duration-2: 0ms; }
}
